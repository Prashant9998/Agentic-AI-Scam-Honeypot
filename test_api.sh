...
# Test 3: Login Endpoint (With Auth)
...
# Test 4: IVR Endpoint (With Auth)
echo "TEST 4: Fake Customer Care IVR"
curl -X POST ${API_URL}/api/ivr \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: ${API_KEY}" \
  -d '{"caller_id":"+919876543210","input":"My account is blocked"}'
echo -e "\n\n"

# Test 5: KYC Endpoint (With Auth)
echo "TEST 5: Fake KYC Verification"
curl -X POST ${API_URL}/api/kyc \
  -H "Content-Type: application/json" \
  -H "X-API-KEY: ${API_KEY}" \
...
curl -X GET "${API_URL}/api/logs?limit=5" \
  -H "X-API-KEY: ${API_KEY}"
echo -e "\n\n"
